...
class Interpreter(NodeVisitor):
    ...
    def visit_Declaration(self, node):
        var_name = node.var_node
        self.declared_variables[var_name] = self.visit(node.right)
        print()
    ...
    def visit_Var(self, node):
        var_name = node.value
        print(var_name)
        var_value = self.declared_variables.get(node.value)
    ...
    def visit_Do_stmt(self, node):
        ...
        while self.visit(node.while_condition):
            self.visit(node.do_stmt) #Assign_stmt, Declaration, Cond_stmt , Var, Program, type, If_stmt, DO_stmt, Body, UnaryOp, Num
...
def main():
    ...
    print(interpreter.declared_variables)
